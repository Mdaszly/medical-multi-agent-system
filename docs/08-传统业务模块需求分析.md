...
> **编制日期**：2026-05-18
>
> **更新说明**：V1.2 - 补充药师角色相关内容，包括药师角色定义、权限矩阵、药师管理系统模块、处方审核与发药流程等
>
...
| 术语              | 定义                                    |
| --------------- | ------------------------------------- |
| **User（用户）**    | 平台注册用户，包括普通患者、医生、药师和管理员，通过userRole字段区分 |
| **Patient（患者）** | userRole=user的普通用户，通过平台进行问诊预约         |
| **Doctor（医生）**  | 平台认证的执业医师，提供诊疗服务                      |
| **Pharmacist（药师）** | userRole=pharmacist的药房工作人员，负责处方审核和药品发放 |
...
│   ├── PharmacistController.java    # 药师端接口
...
│   ├── PharmacistService.java       # 药师业务接口
│   ├── PharmacistServiceImpl.java   # 药师业务实现
...
│   ├── PrescriptionMapper.java      # 处方数据库访问
│   ├── DrugMapper.java              # 药品数据库访问
...
| <br />     | 处方查看   | 查看医生开具的处方（患者可在个人中心查看本人处方）   |
| <br />     | 病历查看   | 查看电子病历                            |
| **费用支付**   | 账单查询   | 查看待支付账单                           |
| <br />     | 在线支付   | 完成费用支付                            |
| <br />     | 支付记录   | 查看支付历史                            |
...
| 功能模块      | 功能点       | 说明         |
| --------- | --------- | ---------- |
| **药师登录**  | 账号密码登录    | 药师专属登录入口   |
| <br />   | 退出登录      | 清除登录状态      |
| **处方管理**  | 待审核处方查询   | 查看待审核的处方列表  |
| <br />   | 处方审核      | 审核医生开具的处方   |
| <br />   | 待发药处方查询   | 查询待发药的处方    |
| <br />   | 处方发药      | 执行发药操作      |
| <br />   | 已完成处方查询   | 查询已完成发放的处方  |
| **药品管理**  | 药品库存查询    | 查看药品库存情况    |
| <br />   | 药品信息管理    | 管理员/药师管理药品信息 |
| **数据统计**  | 发药统计      | 统计日/周/月发药情况  |
...
| 接口路径                              | 方法   | 功能        | 权限     |
| --------------------------------- | ---- | --------- | ------ |
| `/api/prescription/list/pending-dispense` | GET  | 待发药处方列表   | 药师/管理员 |
| `/api/prescription/dispense`       | POST | 处方发药      | 药师/管理员 |
| `/api/prescription/status/update`   | POST | 更新处方状态    | 药师/管理员 |
| `/api/drug/list`                   | POST | 药品列表（带价格） | 药师/管理员 |
| `/api/drug/get`                    | GET  | 药品详情      | 药师/管理员 |
| `/api/drug/fee/calculate`          | POST | 计算药品费用    | 药师/管理员 |
...
CREATE TABLE IF NOT EXISTS user (
    ...
    user_role       VARCHAR(20) NOT NULL DEFAULT 'user' COMMENT '角色：user/doctor/pharmacist/admin',
...
| 药师    | pharmacist   | 处方审核、药品发放、药品管理   |
| 患者    | user         | 挂号预约、查看诊疗记录、支付   |

### 9.2 权限矩阵

| 功能      | 超级管理员 | 运营管理员 | 审核管理员 | 普通管理员 | 医生 | 药师 | 患者 |
| ------- | ----- | ----- | ----- | ----- | -- | -- | -- |
| 用户列表查看  | ✅     | ✅     | ✅     | ❌     | ❌  | ❌  | ❌  |
| 用户详情查看  | ✅     | ✅     | ✅     | ❌     | ❌  | ❌  | ❌  |
| 用户信息编辑  | ✅     | ✅     | ❌     | ❌     | ❌  | ❌  | ❌  |
...
| **P1** | 审计日志功能     | 中   | 合规必需    |
| **P1** | 角色权限管理     | 中   | 权限精细化控制 |
| **P1** | 药师登录/角色权限   | 中   | 药品管理核心功能 |
| **P1** | 处方审核/发药功能   | 中   | 药师核心业务  |
| **P2** | 系统配置管理     | 小   | 运维便利性   |
| **P2** | 登录日志分析     | 小   | 安全监控    |
| **P2** | 药品管理       | 中   | 药品管理功能  |
| **P3** | 数据统计看板     | 小   | 运营增值功能  |

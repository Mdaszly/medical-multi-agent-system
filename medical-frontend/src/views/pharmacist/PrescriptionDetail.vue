...
import { getPrescriptionById, dispensePrescription } from '@/services/medical/chufangguanli'
...
const handleDispense = async () => {
  ...
  try {
    await dispensePrescription({ prescriptionId: prescription.value.id })
...
<template>
  <div class="prescription-detail-page" v-loading="loading">
    <el-card v-if="prescription">
      ...
      <div class="action-section" v-if="prescription.status === 1">
        <el-button type="primary" size="large" :loading="dispensing" @click="handleDispense">
          确认发药
        </el-button>
      </div>
      
      <div class="action-section" v-else-if="prescription.status >= 2">
        <el-tag type="success" size="large">已完成发药</el-tag>
      </div>
    </el-card>
